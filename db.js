...
// --- PERSISTENCE FIX ---
// Use environment variable DB_VOLUME_PATH (e.g., /var/data) for persistent storage on Render Disk.
// Falls back to the local './data' directory if the variable is not set (non-persistent).
const DATA_ROOT = process.env.DB_VOLUME_PATH || path.join(process.cwd(), 'data');
if (!fs.existsSync(DATA_ROOT)) fs.mkdirSync(DATA_ROOT, { recursive: true });

export const db = new Database(path.join(DATA_ROOT, 'x402.db'));
